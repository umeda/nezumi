...
def print_frame2(f):
    ...
    print(' ')
    print(' ')
    print("Raw Frame = {}".format(msg))
    # print("Hex Frame={}".format(binascii.hexlify(msg)))
    try:
        ...
        b = randint(1, 100)
        lcd.color = [r,g,b]
        lcd.color = [128,128,int(b)]
        lcd.color = colors[randint(0,6)]
        lcd.clear()
        lcd.message = parsed_msg['from']
        # print('zuul')
        # print(str(type(f)))
        # print(aprslib.parse(f.decode('utf-8')))
        time_str = datetime.now().strftime('%H:%M')
        lcd.cursor_position(11,0)
        lcd.message = time_str
        url = 'https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=' + str(parsed_msg['latitude']) + '&longitude=' + str(parsed_msg['longitude']) + '&localityLanguage=en'
        print(url)
        resp = get(url)
        regex = re.compile('[^a-zA-Z ]')
        city_sanitized = regex.sub('', json.loads(resp.text)['city'])
        print(city_sanitized)
        lcd.cursor_position(0,1)
        lcd.message = city_sanitized
...
def main():
    ...
    global colors
    global color
    colors  = [[100, 0, 0], [0, 100, 0], [0, 0, 100], [0,50,50], [50,0,50], [50,50,0], [33,33,33]]
    color = 1
    lcd.clear()
