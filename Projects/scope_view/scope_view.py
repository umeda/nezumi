...
def findScope():
    print('searching for dso')
    portsLst = scanSerialPorts()    #scan available ports and store in variable
    # for i in range(0, len(portsLst)):
    #    print("(index) {0:d}     (port) {1:s}".format(i, portsLst[i]))
    # portNdx = int(input("Enter port index [0 - 5] for DSO150:   "))
    global ser
    for portNdx in range(0, len(portsLst)):
        ser = serial.Serial()
        ser.baudrate = baud
        ser.port = portsLst[portNdx]
        ser.timeout = 1
        ...
        foundScope = False
        while(gotData):
            recData = ser.readline().decode()
            scopeData.append(recData)
            if 'RecordLength' in recData: 
                scopeNdx = portNdx
                foundScope = True
            if len(recData) == 0: gotData = False
        print('characters received = ' + str(len(scopeData)))
        if foundScope: break
        ser.close()
    print('pau search')
    return scopeNdx    
